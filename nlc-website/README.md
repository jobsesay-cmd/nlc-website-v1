...
# 4) Start PostgreSQL service from Compose (must match service key in docker-compose.yml)
...
pnpm dev
```
> **Important:** The service name in `docker compose up -d <service>` must exactly match the key under `services:` in `docker-compose.yml`. In this README example, the correct DB service is `db` (not `postgres`).
