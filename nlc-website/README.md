...
## 26) Local Development Quickstart (App Scaffold)
...
Environment values are validated at runtime in `src/lib/env.ts`.

### npm 403 Troubleshooting
If `npm install` returns `403 Forbidden`, your network likely blocks public npm registries.
Use one of these options:
1. Configure an approved internal npm registry and set `NPM_CONFIG_REGISTRY`.
2. Export corporate proxy variables correctly (`HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`).
3. If your org requires a token/allowlist, authenticate and request access for required packages.
